...
@section('title', 'Halaman Data Pinjam')
@include('part.navbar')
<!doctype html>
<html lang="en">
<head>
...
<body>
            ...
            <div class="row justify-content-end"> <!-- Mengubah kelas justify-content-right menjadi justify-content-end -->
                <div class="col-3">
                    <div class="input-group mb-3">
                        <a href="/pengunjung/cetakk" class="btn btn-primary"><i class="fa-solid fa-print"></i> Cetak</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="container-s">
        <div class="row justify-content-center">
            <div class="col-md-6">
                <form action="/datapinjam" enctype="multipart/form-data">
                    <div class="input-group mb-3">
        ...
        <table id="table" class="table table-bordered table-hover">
            ...
            <tr>
                ...
                <th style="width: 15%;">Nama <i class="fas fa-sort sortable-icon" onclick="sortTable(1)"></i></th>
                <th style="width: 15%;">Nama Buku  <i class="fas fa-sort sortable-icon" onclick="sortTable(2)"></i></th>
                <th style="width: 10%;">Alamat <i class="fas fa-sort sortable-icon" onclick="sortTable(3)"></i></th>
        ...
        <tbody>
            @foreach ($peminjamen as $peminjam)
            <tr>
                <td>{{ ($peminjamen->currentPage() - 1) * $peminjamen->perPage() + $loop->iteration }}</td>
                <td>{{ $peminjam->nama_peminjam }}</td>
                <td> {{ $peminjam->bukus->judul }}</td>
                <td>{{ $peminjam->alamat }}</td>
                <td>{{ $peminjam->no_hp }}</td>
                <td>{{ $peminjam->tanggal_pinjam }}</td>
                <td>{{ $peminjam->tanggal_wajib_kembali }}</td>
                <td>{{ $peminjam->tanggal_pengembalian }}</td>

                <td>
                    <form action="/datapinjam/{{ $peminjam->id }}" method="post" class="d-inline">
                        @method('POST')
                        @csrf
                        <button type="submit" class="btn btn-danger" onclick="return confirm('BALIKAN BUKU?')">
                           </i> PENGEMBALIAN
                        </button>
                    </form>
                </td>
